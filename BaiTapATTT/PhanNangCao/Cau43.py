...
def GenerateRandomPrimeNumber(k, t):
    while True:
        ...
        flag = True
        listPr = sieveOfEratosthenes(int(res / 2))
        for i in listPr:
            if res % i == 0:
                flag = False
                break
...
def main():
    ...
    while True:
        ...
        if 0 < n < 1000:
            ...
            print("Đầu vào không hợp lệ, hay nhập lại!!!")
    while True:
        p = GenerateRandomPrimeNumber(7, 10)
        if p < 100:
            break
    for i in range(100):
        if checkPrime(ExponentialSquaring(i, p, n)):
            listPr.append(i)
    print(f"p = {p}")
    if len(listPr) != 0:
        print(f"Tập các số thoả yêu cầu là: {listPr}")
    else:
        print("Không tìm thấy số thoả yêu cầu!!!")
